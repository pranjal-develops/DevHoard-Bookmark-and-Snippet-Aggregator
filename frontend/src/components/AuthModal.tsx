...
// import { XMarkIcon } from '@heroicons/react/24/outline'; // or any close icon
    ...
    const handleAuth = async (e: React.FormEvent) => {
        ...
        try {
            const res = await axios.post(`http://localhost:8080${url}`, {
                username,
                password,
                guestId // 👈 THE CONSOLIDATION SIGNAL
            });
 ...
 */}
    ...
    dark:bg-white/5 dark:border-white/10 dark:text-white dark:focus:border-lime-500"
                    ...
                    {error && (
                        <p className={`text-center text-sm ${error.includes('Success') || error.includes('created') ? 'text-lime-500' : 'text-red-400'}`}>
                            {error}
                        </p>
                    )}
                ...
                <p className="mt-6 text-center text-xs text-zinc-500">
                    By continuing, you agree to the DevHoard Archive Protocols.
                </p>
            </div>
        </div>
    );
